...
llama3_70b = ChatGroq(groq_api_key=os.getenv("GROQ_API_KEY"),temperature=0, model_name="llama-3.3-70b-versatile")
mixtral_8x7b = ChatGroq(temperature=0, model_name="mixtral-8x7b-32768")
...
def truncate_input(text: str, max_tokens: int = 1500) -> str:
...
async def generate_report_plan(state: ReportState, config: RunnableConfig):
    ...
    structured_llm = llama3_70b.with_structured_output(Queries)
    ...
    # Generate sections 
    structured_llm = llama3_70b.with_structured_output(Sections)
...
def write_section(state: SectionState):
    ...
    #Truncate system instructions to 1000 words (~750 tokens)
    truncated_instructions = truncate_input(system_instructions, max_tokens=750)
    # Generate section  
    section_content = llama3_70b.invoke([SystemMessage(content=truncated_instructions)]+[HumanMessage(content="Generate a report section based on the provided sources.")])
...
def write_final_sections(state: SectionState):
    ...
    # Generate section  
    section_content = llama3_70b.invoke([SystemMessage(content=system_instructions)]+[HumanMessage(content="Generate a report section based on the provided sources.")])
